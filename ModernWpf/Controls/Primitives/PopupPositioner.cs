...
namespace ModernWpf.Controls.Primitives
{
	internal class PopupPositioner : DependencyObject, IDisposable
	{
		...
		internal const double Tolerance = 1.0e-2;
		...
		/// <summary>
		///  Tooltips should show on Keyboard focus. Chooses the behavior of where the Popup should
		///  be placed on screen. Takes into account TreatMousePlacementAsBottom to place tooltips
		///  correctly on keyboard focus.
		/// </summary>
		...
		// To position the popup, we find the InterestPoints of the placement rectangle/point in the
		// screen coordinate space. We also find the InterestPoints of the child in the popup's
		// space. Then we attempt all valid combinations of matching InterestPoints (based on
		// PlacementMode) to find the position that best fits on the screen.
		// NOTE: any reference to the screen implies the monitor for full trust and the browser area
		// for partial trust
		private void UpdatePosition()
		{
			...
			for (int i = 0; i < positions; i++)
			{
				...
				if (placement == PlacementMode.Custom)
				{
					// The custom callback only calculates relative to 0,0 so the placementTarget's
					// top/left need to be re-applied.
				...
				// Find percent of popup on screen by translating the popup bounds and calculating
				// the percent of the bounds that is on screen
				// Note: this score is based on the percent of the popup that is on screen not the
				// percent of the child that is on screen. For certain scenarios, this may produce
				// in counter-intuitive results. If this is a problem, more complex scoring is needed
			...
			// Check to see if the pop needs to be nudged onto the screen. Popups are not nudged if
			// their axes do not align with the screen axes
				...
				Math.Abs(intersection.Height - childBounds.Height) > Tolerance)
			{
				...
				// Create a vector pointing from the top of the placement target to the bottom to
				// determine which direction the popup should be nudged in. If the vector is zero
				// (NaN's after normalization), nudge horizontally
				Vector horizontalAxis = topRight - topLeft;
				horizontalAxis.Normalize();

				// See if target's horizontal axis is aligned with screen (For opaque windows always
				...
				// Create a vector pointing from the top of the placement target to the bottom to
				// determine which direction the popup should be nudged in If the vector is zero
				// (NaN's after normalization), nudge vertically
		...
		// This struct is returned by GetPointCombination to indicate which points on the target can
		// align with points on the child
		...
		// allow errors in double calculations
